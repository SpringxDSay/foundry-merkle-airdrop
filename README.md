...
This project uses merkle proof to allow a list of eligible addresses to claim airdrop

## SetUp

```bash
git clone https://github.com/SpringxDSay/foundry-merkle-airdrop.git
```
...
Running `forge build --zksync` returns a broken pipe error for me

```
god@GODs-MacBook-Air merkle-airdrop %  forge build --zksync
[⠃] Using zksolc-1.5.4
[⠒] Compiling (zksync)
[⠊] Compiling 57 files with zksolc and solc 0.8.25
Error:
Broken pipe (os error 32)
```

This error is an ongoing issue in the zksync repo. However, the compilation works on some other's PC.
